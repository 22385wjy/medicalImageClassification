...
from MydataPreprocess.Dataset_ADNCEMCILMCI_4c import *
import shutil
...
class newClassifier(nn.Module):
    def __init__(self, classCount = 4, p=0.5):
...
def TrainforClassfy(model,train_loader,valid_loader,test_loader,optimizer,scheduler,criterion,p1,p2):
    logger = Logger(os.path.join(p1, 'log.txt'), title='4 classes predict')
    logger.set_names(
...
if __name__=="__main__":
    ...
    val_interval = 1
    random.seed(1)
    imgPath="./ADdata_DTI/ADandNCandEMCIandLMCI_4data/"
    train_loader,valid_loader,test_loader=three_dataLoader(imgPath)

    p1 = "./Out_Mynet/ADNCEMCILMCI_Classify/"
    p2 = p1 + "trained_model/"
